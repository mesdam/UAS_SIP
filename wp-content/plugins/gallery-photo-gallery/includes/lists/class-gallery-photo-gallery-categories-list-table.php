...
class Gpg_Categories_List_Table extends WP_List_Table{
     ...
     * @return mixed
     */
    public static function get_gallery_categories( $per_page = 20, $page_number = 1 ) {
    ...
    public function get_gallery_category( $id ) {
        ...
        $sql = "SELECT * FROM {$wpdb->prefix}ays_gallery_categories WHERE id=" . absint( intval( $id ) );
  ...
  /*  
    ...
    function column_title( $item ) {
        ...
        $restitle = Gallery_Photo_Gallery_Admin::ays_restriction_string("word",stripcslashes($item['title']), 5);
        $title = sprintf( '<a href="?page=%s&action=%s&gallery_category=%d"><strong>%s</strong></a>', esc_attr( $_REQUEST['page'] ), 'edit', absint( $item['id'] ), $restitle );
    ...
    function column_items_count( $item ) {
        global $wpdb;
        $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}ays_gallery WHERE categories_id = " . $item['id'];
    ...
    public function prepare_items() {
        ...
        $this->items = self::get_gallery_categories( $per_page, $current_page );
    }
