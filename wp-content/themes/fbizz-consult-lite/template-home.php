...
for ($k = 1; $k <= 4; $k++) {
    ?>
    <?php $boxid    = absint(get_theme_mod('fbizz_consult_lite_box' . $k)); ?>
    ...
    if ($boxquery->have_posts() && $boxid > 0) :
        while ($boxquery->have_posts()) : $boxquery->the_post();
            ...
            $image         = esc_url(get_the_post_thumbnail_url($post->ID, 'fbizz-consult-lite-home-box-size'));
            $title         = esc_html(get_the_title($post->ID));
            $my_postid     = $post->ID;
            $content       = esc_html(fbizz_consult_lite_get_excerpt($my_postid, '250'));
                ...
                <div class="fourbox">
                    ...
                    <div class="mainresourcebox">
                        ...
                        <div class="resourcebox"><p><?php
                                ...
                                echo esc_html(fbizz_consult_lite_get_excerpt($boxid, 150));
                                ?></p><a class="rdmore" href="<?php echo esc_url($boxvariables['url']); ?>"><?php echo esc_html__('Read More', 'fbizz-consult-lite') ?></a>
                        </div>
                    </div><!--mainresourcebox-->
                </div>
            <?php } ?>
